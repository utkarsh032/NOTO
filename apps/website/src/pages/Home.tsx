...
function Hero() {
  ...
  return (
    <div className="border-default bg-surface-secondary border-b">
      <Container className="py-20 text-center sm:py-28">
        <h1 className="text-primary mx-auto max-w-3xl text-4xl font-semibold tracking-tight text-balance sm:text-6xl">
          Your notes. Your workspace.
        </h1>

        <p className="text-secondary mx-auto mt-6 max-w-2xl text-lg text-pretty">
  ...
  return (
    <Section>
      <div className="grid gap-6 md:grid-cols-3">
        {HIGHLIGHTS.map((item) => (
          <Card key={item.title}>
            <h2 className="text-primary text-base font-semibold">{item.title}</h2>
        ...
        {[
          ...
          <Card key={platform.name} className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
              <h3 className="text-primary text-sm font-semibold">{platform.name}</h3>
              {platform.planned ? <Badge>Planned</Badge> : null}
            </div>
            <p className="text-secondary text-sm">{platform.detail}</p>
          </Card>
        ))}
      </div>
