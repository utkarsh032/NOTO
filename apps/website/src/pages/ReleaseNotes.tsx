    ...
    <>
      ...
      <Section>
        {state.status === 'loading' ? (
          <Card>
            <p className="text-muted text-sm">Loading releases…</p>
          </Card>
        ) : null}

        {state.status === 'unavailable' ? (
          <Card>
            <h2 className="text-content text-base font-semibold">Releases could not be loaded</h2>
        ...
        {state.status === 'ready' && state.releases.length === 0 ? (
          <Card>
            <h2 className="text-content text-base font-semibold">Nothing published yet</h2>
            <p className="text-muted mt-2 text-sm">
              ...
              return (
                <Card key={release.tag}>
                  <div className="flex flex-wrap items-center gap-3">
                    <h2 className="text-content text-lg font-semibold">{release.name}</h2>
                    {release.prerelease ? <Badge tone="accent">Prerelease</Badge> : null}
                    {published ? <span className="text-subtle text-sm">{published}</span> : null}
                  </div>
