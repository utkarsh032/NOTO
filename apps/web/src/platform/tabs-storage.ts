...
/**
 * Persists which documents are open to localStorage.
 *
 * The store is platform-agnostic, so the read/write pair lives here — the same
 * arrangement the settings use. Ids only: the documents themselves come back
 * from IndexedDB, and a tab whose document has since gone is dropped when the
 * list loads.
 */
...
function readTabs(): TabsState {
  try {
    ...
    // Validated rather than trusted: this is the one piece of shell state a
    // user could plausibly have hand-edited, and a malformed tab list would
