...
Each of those produces **one APK per ABI**, not one APK:

```
app-arm64-v8a-release.apk     # every phone made since about 2016
app-armeabi-v7a-release.apk   # older 32-bit phones
app-x86_64-release.apk        # emulators
app-x86-release.apk           # emulators
```

A single APK carrying all four is what made the published download 104 MB, so
`with-android-app-size.cjs` turns that off — see [why the APK is split by
ABI](#why-the-apk-is-split-by-abi). `installDebug` still picks the right one
for whatever device is attached, so day to day this changes nothing.
...
The result is `app\build\outputs\apk\release\app-arm64-v8a-release.apk`. Then
either push it over USB, with the phone in developer mode:
...
### Why the APK is split by ABI

The Expo template builds one universal APK holding `armeabi-v7a`, `arm64-v8a`,
`x86` and `x86_64`, and stores those libraries uncompressed. React Native,
Hermes, fbjni, Fresco, reanimated, worklets, the Expo modules and SQLite
therefore shipped four times over — about three quarters of a 104 MB download
that no single phone could use.

`apps/mobile/plugins/with-android-app-size.cjs` fixes that in the generated
project, since `apps/mobile/android` is regenerated by `expo prebuild` and
cannot be hand-edited. It sets `splits.abi` so each ABI gets its own APK, and
turns on four release-build properties: R8 (`enableMinifyInReleaseBuilds`),
resource shrinking, JS bundle compression, and `expo.useLegacyPackaging` so
the native libraries are compressed. It also drops Fresco's GIF and WebP
codecs, which the mobile UI has no images to use.

Measured on a clean prebuild and `assembleRelease`:

| APK           | before | after   |
| ------------- | ------ | ------- |
| `arm64-v8a`   | 104 MB | 16.0 MB |
| `armeabi-v7a` | —      | 15.1 MB |

Three consequences worth remembering:

- **R8 is on for release builds.** It strips unreachable code, and anything
  reached only by reflection needs a Proguard rule to survive. CI has no device
  test, so a release build wants a smoke test on a real phone before it is
  published. If a crash reproduces only on a release build, rule R8 out first
  with `-Pandroid.enableMinifyInReleaseBuilds=false`.
- **`bundleRelease` must disable splits.** Play splits the bundle per device
  itself, so the release workflow passes `-Pnoto.enableAbiSplits=false` to that
  task.
- **Compressed libraries trade install size for download size.** Android's
  default is to store `.so` files uncompressed so they can be mapped straight
  out of the APK; compressing them means Android extracts them at install, so
  the phone holds them twice. The APK on the download page is something people
  wait for over a mobile connection, which is what tips the balance. The Play
  Store is unaffected — it ships the AAB and re-packs per device.
